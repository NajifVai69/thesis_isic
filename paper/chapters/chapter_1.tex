...
together rather than separately. We argue that a carefully designed lightweight
hybrid, fed with the same contextual metadata a clinician uses, can match or
beat much larger image-only baselines while staying small enough to deploy. We
...
\begin{enumerate}
  ...
  \item To quantify the contribution of each design choice (the CNN stem, the
        transformer trunk, the metadata head, the dual-backbone fusion, and the
        KAN head) through controlled ablations, and to report efficiency
        alongside accuracy.
\end{enumerate}
